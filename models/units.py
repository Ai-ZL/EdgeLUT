...
class SDYEHOUnit(nn.Module):
    rot_dict = {'s': [0, 1, 2, 3], 'd': [0, 1, 2, 3], 'y': [0, 1, 2, 3], 'e': [0, 1, 2, 3], 'h': [0, 1, 2, 3], 'o': [0, 1, 2, 3]}
    pad_dict = {'s': (0, 3, 0, 3), 'd': (0, 3, 0, 3), 'y': (0, 3, 0, 3), 'e': (0, 3, 0, 3), 'h': (0, 3, 0, 3), 'o': (0, 3, 0, 3)}
    avg_factor = 6.

    def __init__(self, ktype, nf=64, upscale=4, act=nn.ReLU):
        super(SDYEHOUnit, self).__init__()
        self.ktype = ktype
        self.upscale = upscale
        self.act = act()
    ...
    def dconv_forward(self, x):
        ...
        if self.ktype == 's':
            ...
            x = torch.cat([x[:, :, 0, 0], x[:, :, 0, 3], x[:, :, 3, 0], x[:, :, 3, 3]], dim=1)
        elif self.ktype == 'h':
            x = torch.cat([x[:, :, 0, 0], x[:, :, 2, 2], x[:, :, 2, 3], x[:, :, 3, 2]], dim=1)
        elif self.ktype == 'o':
            x = torch.cat([x[:, :, 0, 0], x[:, :, 2, 2], x[:, :, 1, 3], x[:, :, 3, 1]], dim=1)
        # elif self.ktype == 'l':
        #     x = torch.cat([x[:, :, 0, 0], x[:, :, 0, 1], x[:, :, 1, 1]], dim=1)
        else:
            raise AttributeError
