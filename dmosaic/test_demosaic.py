...
from data_demosaic import SRBenchmark
...
def parse_args():
    ...
    parser.add_argument("--test-dir", type=str, default='/home/zlai/dataset/benchmark',
                        help="Testing images")
    ...
    parser.add_argument('--msb', type=str, default='hdb', choices=['p', 'hl', 'hd', 'hdb', 'hdv', 'hdbv', 'hdbl'])
    parser.add_argument('--lsb', type=str, default='hdv', choices=['p', 'hl', 'hd', 'hdb', 'hdv', 'hdbv', 'hdbl'])
    ...
    with torch.no_grad():
        ...
        for j in range(len(test_datasets)):
            ...
            for k in range(len(files)):
                ...
                psnrs.append(bPSNR(y_gt, y_out, sr_scale))

            print('Dataset {} | AVG LUT bPSNR: {:.4f}'.format(test_datasets[j], np.mean(np.asarray(psnrs))))
