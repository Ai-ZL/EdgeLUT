...
def _yuv2rgb(img, maxVal=255):
    if isinstance(img, torch.Tensor):
        # 处理 PyTorch tensor
        O = torch.tensor([[0], [128], [128]], device=img.device, dtype=img.dtype)
        T = torch.tensor([[1, 0, 1.4022],
                         [1, -0.3456, -0.7145],
                         [1, 1.7710, 0]], device=img.device, dtype=img.dtype)
        ...
        # 重塑回原始形状
        yuv = t.reshape(original_shape)
        
        # 四舍五入和裁剪
        #yuv = torch.round(yuv)
        #yuv = torch.clamp(yuv, 0, 255)
        
        # 转换为 uint8（如果需要的话）
        #if yuv.dtype != torch.uint8:
        #    yuv = yuv.to(torch.uint8)
...
def rgb2yuv(img):
    ...
    img_out[:,:,1] = -0.1687 * img[:,:,0] - 0.3313 * img[:,:,1] + 0.5 * img[:,:,2] + 128
    img_out[:,:,2] = 0.5 * img[:,:,0] - 0.4187 * img[:,:,1] - 0.0813 * img[:,:,2] + 128
    #img_out.round()
    #img_out = np.clip(img_out,0,255)
    #img_out.astype(in_img_type)
    return img_out
